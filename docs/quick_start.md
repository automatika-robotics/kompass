...
robot_config = RobotConfig(
    ...
    geometry_params=np.array([0.5, 0.5, 0.5]),
    ctrl_vx_limits=LinearCtrlLimits(max_vel=1.0, max_acc=1.5, max_decel=2.5),
    ctrl_omega_limits=AngularCtrlLimits(
        max_vel=1.0, max_acc=2.0, max_decel=3.0, max_steer=np.pi
    ),
)
```
...
```{seealso}
Several other configuration options are available for each component, refer to the [Planner](./navigation/path_planning.md) and [Controller](./navigation/control.md) deticated pages for more details.
```
...
```{seealso}
Learn more about the available Launcher options [here](design/launcher.md).
```
