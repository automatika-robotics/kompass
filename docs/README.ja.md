...
- **高速化のためのC++・マルチスレッド・クロスGPU対応**
  すべてのコアアルゴリズムはモダンなC++で記述され、マルチスレッドによる高速なCPU実行をサポートします（[kompass-core](https://github.com/automatika-robotics/kompass-core)）。Kompassは、SYCLベースのGPGPU実行をサポートする初のナビゲーションフレームワークであり、あらゆるGPU上での高速処理を可能にします。
